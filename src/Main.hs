...
showRLG rlg =
  ...
  where
    nonTerminalsStr = convertRLGSymbolsToStr (nonTerminalsRLG rlg)
...
convertRLGRulesToStr (r:rs) =
  convertSingleRLGRuleToStr r ++ "\n" ++ convertRLGRulesToStr rs
...
data RRG =
  RRG
    { nonTerminalsRRG :: [String] --might contain new terminals named "A123", "B21", ...
    , terminalsRRG :: String
...
showRRG rrg =
  ...
  where
    nonTerminalsStr = convertRRGNonterminalsToStr (nonTerminalsRRG rrg)
...
convertRRGNonterminalsToStr (nt:nts) =
  nt ++ "," ++ convertRRGNonterminalsToStr nts
...
convertRRGRulesToStr (r:rs) =
  convertSingleRRGRuleToStr r ++ "\n" ++ convertRRGRulesToStr rs
...
showNFSM nfsm =
  ...
  where
    statesStr = convertNFSMStatesToStr (states nfsm)
...
convertNFSMTransitionsToStr (t:ts) =
  convertSingleNFSMTransitionToStr t ++ "\n" ++ convertNFSMTransitionsToStr ts

convertSingleNFSMTransitionToStr ::
     (Show a1, Show a2) => (a1, Char, a2) -> String
convertSingleNFSMTransitionToStr t =
  show (myFst t) ++ "," ++ [mySnd t] ++ "," ++ show (myThd t)
...
createRLG xs
  ...
  where
    nonTerminals =
      sort (myUnique (removeChar ',' (checkNonTerminalsFormat (head xs))))
    terminals =
      sort (myUnique (removeChar ',' (checkTerminalsFormat (xs !! 1))))
    startingSymbol = checkStartingSymbolFormat (xs !! 2) nonTerminals
    rules =
      sort
        (myUnique
           (map parseRule (checkRulesFormat (drop 3 xs) nonTerminals terminals)))
...
checkNonTerminalsFormat nts
  ...
  | strHasCSVFormat
     ...
     ['A' .. 'Z']
     "item"
     "ERROR: Incorrect format of non-terminal set." = nts
...
checkStartingSymbolFormat str nts
  ...
  | head str `notElem` nts =
    error "ERROR: Starting symbol is non-existing non-terminal."
  | otherwise = head str
...
determineError curr x
  ...
  | curr == "right" && x `elem` ['a' .. 'z'] =
    "ERROR: Rules contain non-existing terminal '" ++ [x] ++ "'."
  | curr == "rightT" && x `elem` ['a' .. 'z'] =
    "ERROR: Rules contain non-existing terminal '" ++ [x] ++ "'."
  | curr == "rightT" && x `elem` ['A' .. 'Z'] =
    "ERROR: Rules contain non-existing non-terminal '" ++ [x] ++ "'."
...
splitRLGRules nonTerminals (rule:rs) =
  if ruleIsRRG (snd rule) --ruleIsRRG only needs the right side of the rule
...
splitSingleRLGRule nonTerminals rule =
  if last (snd rule) `elem` ['A' .. 'Z'] --if the rule ends with nonTerminal
...
splitSingleRLGNonTerminalRule nonTerminals ruleRightSide lastNT =
  if length ruleRightSide == 2
    then (nonTerminals, [(lastNT, ruleRightSide)])
    else ( allNonTerminals
         , (lastNT, head ruleRightSide : freeNonTerminal) : restOfRules)
  where
    (allNonTerminals, restOfRules) =
      splitSingleRLGNonTerminalRule
        (freeNonTerminal : nonTerminals)
        (tail ruleRightSide)
        freeNonTerminal
...
convertToNFSM rrg =
  NFSM
    { states = renameNonTerminals (nonTerminalsRRG rrg) (nonTerminalsRRG rrg)
    , alphabet = terminalsRRG rrg
    , transitions = createTransitions (nonTerminalsRRG rrg) (rulesRRG rrg)
    , startState =
        fromJust $ elemIndex [startingSymbolRRG rrg] (nonTerminalsRRG rrg)
    , finalStates =
...
createTransitions allNonTerminals (r:rs) =
  ...
  where
    fromState = fromJust $ elemIndex (fst r) allNonTerminals
    symbol = head (snd r)
...
createFinalStates allNonTerminals (r:rs) =
  if snd r == "#"
    then fromJust (elemIndex (fst r) allNonTerminals) :
         createFinalStates allNonTerminals rs
    else createFinalStates allNonTerminals rs

parseArgs :: [String] -> IO ()
parseArgs ("-i":xs) = loadInput xs >>= print . loadRLG >> exitSuccess
parseArgs ("-1":xs) =
  loadInput xs >>= print . convertToRRG . loadRLG >> exitSuccess
parseArgs ("-2":xs) =
  loadInput xs >>= print . convertToNFSM . convertToRRG . loadRLG >> exitSuccess
